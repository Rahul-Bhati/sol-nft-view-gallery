import "./App.css";
import React, { useEffect, useState } from "react"; // Import the useEffect hook
...
function App() {
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      headers: {accept: 'application/json', 'x-api-key': '519bc9d901a44d7fad1d2995010305b9'}
    };
      ...
      <header className="App-header">
        {data ? 
          <div className='collection'>
            {
              data.map((item, index) => {
                return <Collections key={index} data={item} />
              })
            }
          </div>
        : <p>Loading...</p>}
